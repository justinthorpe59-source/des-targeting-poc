 ...
 * never writes to system2Store. Same shape as System 1's What-if sandbox:
 * local component state feeds the SAME real engine functions
 * (aggregate()/computeRiskStatuses(), via runScenario() — see
 * engine/scenario.ts) that every other System 2 screen uses on the real
 * data, just with hypothetical inputs instead. Baseline is always computed
 * from the untouched real records, so it's provably identical to what
 * Executive summary shows regardless of what the levers are set to.
 */
export function ScenarioWorkspace() {
  ...
  if (records.length === 0) {
    return (
      <section className="space-y-4">
        <div>
          <h1 className="text-lg font-semibold">Scenario workspace</h1>
  ...
  // baseline-vs-scenario row beneath the DES-wide comparison, since a
  // team/division-scoped change may not move the DES-wide numbers by much
  // on its own.
    ...
    <section className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          ...
          <p className="mt-1 text-xs text-slate-500">DES-wide only — divisions/teams keep their own target as their own goal.</p>
          <input
            type="number"
          ...
          />
          <span className="ml-2 text-xs text-slate-500">£k (baseline: £{round1(baselineGoal)}k)</span>
        </div>
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            ...
            />
            <span className="text-xs text-slate-500">% capacity change</span>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          ...
          <p className="mt-1 text-xs text-slate-500">Unfiltered — applies to every imported record.</p>
          <input
            type="number"
          ...
          />
          <span className="ml-2 text-xs text-slate-500">% target change</span>
        </div>
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          ...
          <p className="mt-1 text-xs text-slate-500">That group's own row only — never cascades to its parent.</p>
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <select
      ...
      <div>
        <h2 className="text-sm font-semibold text-slate-700">DES-wide: baseline vs scenario</h2>
        ...
        <div>
          ...
          <div className="mt-2 overflow-x-auto rounded-lg border border-slate-200">
            <table className="min-w-full divide-y divide-slate-200 text-sm">
              ...
              <tbody>
                <tr className="border-t border-slate-100" data-testid="scoped-baseline-row">
                  <td className="px-3 py-2 font-medium text-slate-500">Baseline</td>
                  <td className="px-3 py-2 text-right tabular-nums text-slate-900">£{round1(baselineScopedRollup.target)}k</td>
                ...
                <tr className="border-t border-slate-100 bg-slate-50" data-testid="scoped-scenario-row">
                  <td className="px-3 py-2 font-medium text-slate-500">Scenario</td>
                  <td className="px-3 py-2 text-right tabular-nums text-slate-900">£{round1(scenarioScopedRollup.target)}k</td>
                  <td className="px-3 py-2 text-right tabular-nums text-slate-900">
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4">
        <h2 className="text-sm font-semibold text-slate-700">Save this scenario</h2>
        <div className="mt-2 flex flex-wrap items-center gap-2">
          <input
            type="text"
        ...
        {savedScenarios.length > 0 && (
          <ul data-testid="saved-scenario-list" className="mt-4 divide-y divide-slate-100 text-sm">
            {savedScenarios.map((s) => (
              <li key={s.id} data-testid="saved-scenario-row" data-scenario-id={s.id} className="flex items-center justify-between py-2">
                <span>
                  ...
                  <span className="text-xs text-slate-500">saved {s.savedAt}</span>
                </span>
                <button
                  type="button"
